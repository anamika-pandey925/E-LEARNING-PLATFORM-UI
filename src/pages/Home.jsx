    ...
    <div className="space-y-24 pb-20 overflow-hidden">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            <div className="relative group max-w-md mx-auto aspect-video lg:aspect-square overflow-hidden rounded-3xl border border-white/10 shadow-2xl shadow-emerald-500/5">
          ...
          >
            ...
            <h2 className="font-poppins font-extrabold text-3xl sm:text-4xl text-slate-100 leading-snug">
            ...
            <ul className="space-y-3.5 pt-2">
              <li className="flex items-center space-x-3 text-slate-300 text-sm md:text-base">
                <span className="w-5 h-5 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 shrink-0">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 bg-slate-950/40 rounded-3xl border border-slate-900">
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <h2 className="font-poppins font-extrabold text-3xl text-slate-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feat, idx) => (
            <motion.div
              key={idx}
              className="glass p-6 rounded-2xl border border-slate-800/80 hover:border-emerald-500/20 hover:shadow-lg transition-all duration-300 text-left space-y-4 group"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: idx * 0.1, duration: 0.5 }}
            >
              <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center text-emerald-400 group-hover:bg-emerald-500 group-hover:text-slate-950 transition-all duration-300 shadow-md">
                {feat.icon}
              </div>
              <h3 className="font-poppins font-bold text-lg text-slate-100 group-hover:text-emerald-400 transition-colors">
                {feat.title}
              </h3>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div 
          className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 px-8 py-12 md:py-16 text-center space-y-6 shadow-2xl shadow-emerald-500/10"
        ...
        >
          ...
          <h2 className="font-poppins font-extrabold text-3xl sm:text-4xl text-slate-950 max-w-3xl mx-auto">
            Ready to Begin Your Educational Journey?
          </h2>
          <p className="text-slate-900 font-medium text-base sm:text-lg max-w-2xl mx-auto">
            Unlock certificate courses in programming, competitive preparation, mathematics, science, and english today.
          </p>
          <div className="pt-2 flex justify-center gap-4">
            <Button to="/courses" variant="secondary" className="!bg-slate-950 !text-slate-100 hover:!bg-slate-900 !border-none">
              Get Started
            </Button>
            <Button to="/signup" variant="outline" className="!border-slate-950 !text-slate-950 hover:!bg-slate-950 hover:!text-slate-100">
              Sign Up Free
            </Button>
          </div>
        </motion.div>
